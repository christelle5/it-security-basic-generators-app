...
class RC5FrameDecode(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.label_filename = ttk.Label(self, text="File to decode: ")
        self.entry_message = ttk.Entry(self)
        self.entry_filename = ttk.Entry(self)

        self.generate_button = ttk.Button(self, text="Decode", command=self.encode_file)
        self.back_button = ttk.Button(self, text="Back to Main Menu",
                                      command=lambda: self.controller.show_frame(MainFrame))
        self.encode_file_button = ttk.Button(self, text="Go to Encode File Page",
                                             command=lambda: self.controller.show_frame(RC5FrameEncode))
        ...
        self.encode_file_button.grid(column=2, row=1, padx=10, pady=5)
    ...
    def encode_file(self):
        ...
        output_filename = f"result_{time_str}" + "_decoded_" + str(file_name)
        result = rc5_obj.decode_file(str(file_name), output_filename)

        if result:
            messagebox.showinfo("Success", "File was successfully decoded. Decoded text was saved "
